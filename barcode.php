...
class Barcode
{
    public function generate(string $text = 'some text', string $filename = 'barcode.jpg', string $mode = 'B'): void
    {
			...
			for ($i = 0; $i < $count; $i++) {
				...
				imagefilledrectangle($image, $x1, ($img_height - 30), $x2, 10, $color);
			}
	
			$center_x = ($img_width - imagefontwidth(5) * strlen($text)) / 2;

			imagestring($image, 5, $center_x, ($img_height - 20), $text, $black);
